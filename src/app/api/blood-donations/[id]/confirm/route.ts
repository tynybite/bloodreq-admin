...
// POST /api/blood-donations/:id/confirm - Confirm a donation offer
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (bloodRequest?.requester_id !== user!.id) {
      return errorResponse('Only requester can confirm donations', 'FORBIDDEN', 403);
    }
    ...
    return successResponse({ status: 'accepted' }, 'Donation confirmed');
  } catch (error) {
