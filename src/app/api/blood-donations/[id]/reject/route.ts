...
// POST /api/blood-donations/:id/reject - Reject a donation offer
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (bloodRequest?.requester_id !== user!.id) {
      return errorResponse('Only requester can reject donations', 'FORBIDDEN', 403);
    }
    ...
    return successResponse({ status: 'rejected' }, 'Donation rejected');
  } catch (error) {
